...
simu <- function(n, m, data_use = NULL){
 ...
 Vuse2 <- diag(rep(solve(killoffdiag(Vpost_naive)), n))
  ...
  gee_non_list =  kgee(x= treatment, y= yob, h = h_value, xgrid= c(A_list, (A_list + 1), (A_list + 0.1), (A_list + 0.01)), id=id, V1= Vuse2)$f
  gee_non_para = gee_non_list[1:length(A_list)]
  gee_non_para_1 = gee_non_list[(length(A_list)+1):(2*length(A_list))]
  gee_non_para_p1 = gee_non_list[(2*length(A_list) + 1):(3*length(A_list))]
  gee_non_para_p2 = gee_non_list[(3*length(A_list) + 1):(4 *length(A_list))]
...
for(i in 1:nsim){
  ...
  save(kgee_curve_data, file = paste0("./output/kgee_curve_", "_data.RData"))   
  # compare
...
save(data_have, file = "./output/data_have.RData")
